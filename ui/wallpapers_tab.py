...
from PyQt6.QtWidgets import (
    ...
    QComboBox, QScrollArea, QGridLayout, QSplitter, QSizePolicy, QFrame, QGraphicsDropShadowEffect
)
from ui.grid_item import GridItemWidget
...
class WallpapersTab(QWidget):
    def __init__(self, wallpaper_engine, download_manager, translator, theme_manager, parent=None):
        ...
        self.dm.extraction_completed.connect(self._on_extraction_completed)
    ...
    def _on_extraction_completed(self, pubfileid: str, success: bool):
        pass
    ...
    def _setup_ui(self):
        try:
            ...
            self.details_panel = DetailsPanel(self.we, self.dm, self.tr, self.theme, self)
            ...
            print(f"[WallpapersTab] Error in _setup_ui: {e}")
            traceback.print_exc()
    ...
    def _on_sort_changed(self, index):
        try:
            ...
            print(f"[WallpapersTab] Error in _on_sort_changed: {e}")
    
    def load_wallpapers(self, columns=None):
        try:
            self._clear_grid()
            
            wallpapers = self.we.get_installed_wallpapers()
            wallpapers = [
            ...
            for wallpaper_path in wallpapers:
                try:
                    item = GridItemWidget(str(wallpaper_path), item_size, self)
                    ...
                    print(f"[WallpapersTab] Error creating grid item for {wallpaper_path}: {e}")
                    continue
            ...
            print(f"[WallpapersTab] Error in load_wallpapers: {e}")
            traceback.print_exc()
    ...
    def _calculate_columns(self) -> int:
        try:
            ...
            print(f"[WallpapersTab] Error in _calculate_columns: {e}")
            return 4
    ...
    def _calculate_item_size(self, columns: int) -> int:
        try:
            ...
            print(f"[WallpapersTab] Error in _calculate_item_size: {e}")
            return 185
        ...
        try:
            for item in self.items[:]:
                if hasattr(item, 'release_resources'):
                    try:
                        ...
                        print(f"[WallpapersTab] Error releasing resources: {e}")
            ...
            while self.grid_layout.count():
                try:
                    ...
                    print(f"[WallpapersTab] Error removing widget: {e}")

            self.items.clear()
            
        except Exception as e:
            print(f"[WallpapersTab] Error in _clear_grid: {e}")
            traceback.print_exc()
    ...
    def _update_info(self, count: int, total_size: int):
        try:
            ...
            print(f"[WallpapersTab] Error updating info labels: {e}")
    
    def _on_item_clicked(self, folder_path: str):
        try:
            if not Path(folder_path).exists():
                print(f"[WallpapersTab] Folder does not exist: {folder_path}")
                return
            
            self.selected_folder = folder_path
            self.details_panel.set_folder(folder_path)
            
        except Exception as e:
            print(f"[WallpapersTab] Error in _on_item_clicked: {e}")
            traceback.print_exc()
    ...
    def refresh(self):
        try:
            ...
            print(f"[WallpapersTab] Error in refresh: {e}")
            traceback.print_exc()
    ...
    def resizeEvent(self, event):
        try:
            ...
            print(f"[WallpapersTab] Error in resizeEvent: {e}")
    
    def _recalculate_grid(self):
        try:
            columns = self._calculate_columns()
            selected = self.selected_folder
            self.load_wallpapers(columns=columns)

            if selected and Path(selected).exists():
                self._on_item_clicked(selected)
                
        except Exception as e:
            print(f"[WallpapersTab] Error in _recalculate_grid: {e}")
            traceback.print_exc()
        ...
        try:
            if hasattr(self, 'details_panel') and self.details_panel.folder_path == folder_path:
                try:
                    ...
                    print(f"[WallpapersTab] Error releasing details panel resources: {e}")
                ...
                if hasattr(item, 'folder_path') and item.folder_path == folder_path:
                    if hasattr(item, 'release_resources'):
                        try:
                            ...
                            print(f"[WallpapersTab] Error releasing item resources: {e}")

            if self.selected_folder == folder_path:
                self.selected_folder = None
                
        except Exception as e:
            print(f"[WallpapersTab] Error in release_resources_for_folder: {e}")
            traceback.print_exc()
